...
# --- FILE PATH CONFIGURATION & API HOST ---
...
# Attempt API fetch, fallback to local JSON data if API is offline (Cloud mode)
...
if not subs_data:
    subs_data = {"subscribers": load_json(SUBSCRIBERS_FILE, [])}

with col_status:
    if fetch_api("stats"):
        st.success("🟢 API ONLINE")
    else:
        st.success("🟢 CLOUD OBSERVATORY (Standalone)")
    
    last_run_time = last_run_data.get('last_run') or last_run_data.get('timestamp', '--')
...
if stats_data:
    ...
    c4.metric("Subscribers", len(subs_data.get('subscribers', [])))
else:
...
with chart_col2:
    ...
    if compare_data:
        ...
        prev = load_json(PREV_STATS_FILE, {'total': 0, 'unique_keywords': 0})
        curr = stats_data
...
with action_col:
    ...
    if st.button("⟳ Run Data Pipeline", use_container_width=True, type="primary"):
        with st.spinner("Harvesting data across streams..."):
            res = fetch_api("run_pipeline", method="POST")
            if res:
                st.toast(res.get('message', 'Pipeline triggered successfully!'))
                st.rerun()
            else:
                # Cloud native execution fallback
                try:
                    import main
                    if hasattr(main, "main"):
                        main.main()
                    st.success("✅ Pipeline executed successfully in cloud!")
                    st.rerun()
    ...
    with btn_a:
        if st.button("📧 Test Transmission", use_container_width=True):
            ...
            if res:
                st.toast("Test transmission sent!")
            else:
                try:
                    import email_agent
                    if hasattr(email_agent, "main"):
                        email_agent.main()
                    st.success("✅ Test transmission sent via cloud agent!")
                except Exception as e:
                    st.error(f"❌ Test transmission failed: {str(e)}")
            
    with btn_b:
        if st.button("📡 Broadcast to All", use_container_width=True):
            res = fetch_api("send_bulk_email", method="POST")
            if res:
                st.toast("Broadcast initiated!")
            else:
                try:
                    import email_agent
                    if hasattr(email_agent, "main"):
                        email_agent.main()
                    st.success(f"✅ Successfully broadcasted to {len(subs_data.get('subscribers', []))} subscribers!")
                    st.balloons()
                except Exception as e:
...
with health_col:
    ...
    if not health_data:
        health_data = load_json(PIPELINE_STATS_FILE, {'last_duration_seconds': 0, 'success_rate': 100})
        
    st.text(f"Last Duration: {health_data.get('last_duration_seconds', health_data.get('last_duration', 0))}s")
...
with sub_col2:
    if st.button("Activate Subscription", use_container_width=True):
        if new_sub_email and "@" in new_sub_email:
            ...
            res = fetch_api("subscribe", method="POST", payload={"email": email_clean})
            
            if res and res.get("success"):
                st.success(f"✅ {res.get('message', 'Subscribed successfully!')}")
                st.balloons()
            else:
                # Local fallback subscription handler
...
with unsub_col2:
    if st.button("Unsubscribe", use_container_width=True):
        if unsub_email and "@" in unsub_email:
            ...
            res = fetch_api("unsubscribe", method="POST", payload={"email": email_clean})
            
            if res and res.get("success"):
                st.success(f"✅ {res.get('message', 'Successfully unsubscribed.')}")
            else:
                # Local fallback unsubscription handler
                subscribers = load_json(SUBSCRIBERS_FILE, [])
                if email_clean in subscribers:
